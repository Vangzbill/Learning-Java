...
public class DoubleLinkedLists {
    ...
    public void add(int item, int index)throws RuntimeException{
        if(isEmpty()) {
            addFirst(item);
        } else if(index < 0 || index > size){
            throw new RuntimeException("Nilai indeks di luar batas");
        } else {
            Node current = head;
            int i = 0;
            while(i < index){
    ...
    public void removeFirst() throws RuntimeException {
        if(isEmpty()){
            throw new RuntimeException("Linked List masih kosong, tidak dapat dihapus!");
        } else if (size == 1){
            removeLast();
        } else{
            head = head.next;
    ...
    public int getFirst() throws RuntimeException {
        if(isEmpty()){
            throw new RuntimeException ("Linked List kosong");
        }
        return head.data;
    }
